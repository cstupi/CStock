...
module.exports = [
{
    ...
    config: {
        ...
        handler: async (req, res) => {
            ...
            let order = Order.Get(trans.Order);
            if(order.Owner !== req.auth.credentials.Id)
                return req.response().code(401);
            ...
            if(order.TransactionType == 'SELL'){
                ...
                await Buy(order.Owner, order.Game, trans.Count, trans.Price);
                await Order.UpdateStatus(order.Id, 'COMPLETE');
            } else if(order.TransactionType == 'SELL'){
                await Buy(order.Owner, order.Game, (-1 * trans.Count), trans.Price);
                await Order.UpdateStatus(order.Id, 'COMPLETE');
