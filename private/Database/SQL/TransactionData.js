...
transaction.GetAllTransactionsForGame = function(gameid, callback, failure){
	connection.query('SELECT TransactionId, tran.UserId, t.TransactionType, Asset, Count, Price, Date, m.Status, Order' +
...
transaction.GetAllTransactionsForUserGame = function(userid, gameid, callback, failure){
	connection.query('SELECT TransactionId, UserId, t.TransactionType, Asset, Count, Price, Date, m.Status, Order' +
		' FROM Transaction tran JOIN TransactionType t ON tran.TransactionType = t.TransactionTypeId' +
...
transaction.AddTransaction = function(userid, type, asset,count,price,date, status,order, gameid, callback, failure){
	connection.query('INSERT INTO Transaction (userid, transactiontype,asset,count,price, date,status,order,gameid)'+
		' VALUES (?,?,?,?,?,?,?,?,?)', [userid,type,asset,count,price,date,status,order,gameid], function(error){
		if(error){
			if(failure)
				failure(error);
			throw error;
